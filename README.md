...
Тепер розглянемо саме тіло методу.
...
Тут варто прояснити два моменти:

1. Трохи небезпечно писати конструкцію `String string = null` (опустимо назву змінної, хоча зауважу, що не варто так писати). В такому випадку, краще позбутися перевірки на `imb < 18.5` і зробити так, щоб значенням за замовчуванням було **"Deficit"**.
2. Замість ініціалізації ми можемо зразу повертати те чи інше значення. Це легше читати і створює умови для раннього виходу з методу.

**Але**, думаю, варто зробити рефакторінг дещо глобальніше. Справа в тому, що ціль цього класу практично виконана — ми повертаємо числове значення, тобто індекс. **Але** щоб будь-хто міг зрозуміти, про що йде мова, варто це показати певними словами, тобто позначеннями.
